...
2. 普通小程序开发先复制一份本地配置文件。`config.example.js` 只是模板说明，运行时不会读取；`config.local.js` 已加入 `.gitignore`，不会提交到公开仓库：

```powershell
Copy-Item miniprogram\config.example.js miniprogram\config.local.js
```

3. 打开 `miniprogram/config.local.js`，按注释填写 `apiBase`。值必须包含后端统一前缀 `/api/v1`；体验版/正式版必须使用 HTTPS，开发版可显式配置 localhost：
