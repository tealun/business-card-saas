...
Page({
  data:{token:"",displayName:"",submitting:false,result:"",error:"",loading:true,company:{name:"企业",shortName:"",initial:"企",logoUrl:"",websiteUrl:"",address:"",publicId:"",notificationTemplateId:""}},
  /**
  ...
  async loadPreview(){
    if(!this.data.token){this.setData({loading:false,error:"加入码缺失或已失效，请联系企业管理员重新获取。"});return;}
    try{await ensureSession();const company=await request(`/local-enterprises/join-preview/${encodeURIComponent(this.data.token)}`);company.initial=String(company.shortName||company.name||"企").slice(0,1);this.setData({company,loading:false});}
    catch(error){this.setData({loading:false,error:error&&error.message?error.message:"邀请信息加载失败，请联系企业管理员。"});}
  },
  /**
   * 更新申请人姓名草稿。
   */
  ...
  async submit(){
    ...
    try{
      ...
      await ensureSession();
      const submitted=await request("/local-enterprises/join-requests",{method:"POST",data:{join_token:this.data.token,display_name:displayName}});
      if(notificationAccepted&&submitted.requestId){await request(`/local-enterprises/join-requests/${encodeURIComponent(submitted.requestId)}/notification-subscription`,{method:"POST",data:{template_id:this.data.company.notificationTemplateId}}).catch(()=>null);}
      this.setData({result:"申请已提交"});
