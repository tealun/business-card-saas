...
| ID | Status | Title | File | Line | Evidence | Fix |
|----|--------|-------|------|------|----------|-----|
| SEC-P1-1 | Fixed | Admin bearer token persisted in localStorage | `admin/app.js` | 3, 519, 539 | `localStorage.setItem("bc_admin_token", state.adminToken)` | Moved admin token to `sessionStorage`; token cleared when tab closes |
...
| ID | Status | Title | File | Line | Evidence | Fix |
|----|--------|-------|------|------|----------|-----|
| BE-P1-1 | Fixed | Callback idempotency key depends on raw XML whitespace | `backend/src/wecom/wecom-data-callback.service.ts` | 250-269 | `messageXml.trim()` used in digest; internal formatting changes break idempotency | Fallback digest now uses extracted semantic fields (`openCorpid`, `event`, `changeType`, `TimeStamp`/`CreateTime`, `UserID`, `NewUserID`, `OpenUserID`) instead of raw XML string |
...
| BE-P2-2 | Fixed | Dead-letter alert webhook silently drops failures | `backend/src/wecom/wecom-callback-alert.service.ts` | 25-62 | `catch { return { sent: false ... } }` swallows errors | Log failures and retry with exponential backoff; test added for no-token header path |
| BE-P2-3 | Fixed | Unauthorized tenant callbacks return 400, causing WeCom retries | `backend/src/wecom/wecom-data-callback.service.ts` | 141-144 | `BadRequestException("tenant is not authorized")` | Return `{ handled: false, tenantId: "" }` so WeCom stops retrying; test updated |
...
| BE-P2-6 | Fixed | WeCom API client discards error bodies | `backend/src/wecom/wecom-api-client.service.ts` | 334-336 | `ServiceUnavailableException("WeCom ... HTTP ${status}")` | Parse response body and include `errcode`/`errmsg` in the exception message |
...
| ID | Status | Title | File | Line | Evidence | Fix |
|----|--------|-------|------|------|----------|-----|
| HLTH-P1-1 | Fixed | Hardcoded WeCom dev secrets in source | `backend/src/wecom/wecom-config.service.ts` | 12-19, 66-71 | `devDefaults` suite secrets + `dev-only-wecom-auth-launch-token` | Removed `devDefaults`; all WeCom secrets required via `readRequired` env vars in every environment |
...
- `npm run typecheck`, `npm run lint`, `npm run build`, and `npm audit` are clean.
...
- Coverage thresholds are now met; the branch-coverage blocker is resolved.
