...
- 已补企业授权发起入口：`POST /api/v1/wecom/authorization-links` 由平台方携带 `x-wecom-launch-token` 生成企业微信授权链接，后端负责 `get_pre_auth_code` 与 `set_session_info`，并通过 `GET /api/v1/wecom/authorization-complete` 承接 `redirect_uri` 回跳 `auth_code`；真实授权仍依赖公开 HTTPS 回调 URL 和试点企业管理员操作。
