...
>
> 2026-07-20 架构修订：企业租户不再以企业微信授权为创建前提。平台本地企业、成员、名片和 RBAC 构成标准企业版；企业微信改为可选连接器，承担自动身份识别、通讯录同步、工作台与消息等增强能力。普通微信只能证明自然人身份，不能证明企业归属；企业关系必须通过企业邀请/审批或企业微信授权建立。账号调用许可进入商业化 M0，详见 `01-specs/01_11_Local_Enterprise_And_Identity.md` 与 `02-tasks/02_05_Local_Enterprise_Decoupling_Plan.md`。
...
2. **企业微信是可选增强连接器，不是企业成立条件**：标准企业版使用本地企业、邀请和审批闭环；企业微信连接器负责自动身份识别、通讯录、工作台、客户联系和接口许可管理。
...
当前实施分为两条可独立验收的垂直切片：**本地企业版 M1 是产品主闭环**，企业无需开通企业微信即可创建组织、邀请员工、统一管理内容和名片；**企业微信 M0/M2 是增强连接器轨**，先验证账号调用许可和真实接口成本，再接入自动同步、工作台和消息能力。任何企业微信许可或授权故障不得导致本地后台、企业内容或公开名片整体不可用。
...
| 管理后台 | **M1 当前：原生 HTML/CSS/JavaScript 静态站；目标：React + TypeScript + Vite + Ant Design** | 先保持现有免构建部署，后续按独立迁移计划升级 | Vue、Angular |
...
| 测试 | **当前：Jest（后端）；目标：Playwright（后台 E2E）** | 覆盖率门槛见 §20；静态后台自动化测试尚待补齐 | Mocha、Vitest（后端） |
