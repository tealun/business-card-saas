...
business-card-saas/
  README.md
  backend/                      # 后端 API：NestJS + Fastify + Prisma，独立 npm 子项目
    src/
    prisma/
    scripts/
    package.json
...
- **RLS 与 Prisma（工程硬约束，审计 A4-P1-10）**：所有需要 RLS 的请求**必须在事务内执行**，`SET LOCAL app.tenant_id = <当前租户>`，RLS 策略 `USING (tenant_id = current_setting('app.tenant_id', true)::bigint)`（§15.0，带 missing_ok，无上下文=拒绝而非报错）。落地规则：
