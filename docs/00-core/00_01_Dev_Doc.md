...
- 公开读取名片（仅隐私判定后字段，首次下发 `visit_token`）、记录访问、记录动作（带 `visit_token`，`(visit_id, action_type)` 幂等）、vCard。契约见 01_02 §3.3。
...
- 详情页首次加载下发服务端签发的 `visit_token`，动作上报必须携带。`visit_token` 契约见 §14.6。
...
```text
形态：服务端签发的短期 JWT 或 HMAC opaque token
...
- **RLS 与 Prisma（工程硬约束，审计 A4-P1-10）**：所有需要 RLS 的请求**必须在事务内执行**，`SET LOCAL app.tenant_id = <当前租户>`，RLS 策略 `USING (tenant_id = current_setting('app.tenant_id')::bigint)`（§15.0）。落地规则：
  - 统一用 `TenantTx.run(tenantId, callback)` 包裹：内部开启事务并执行 `SET LOCAL app.tenant_id`（个人上下文另设 `app.account_id`，见 §15.4）。
  - Repository 层**禁止**直接使用裸 Prisma client 访问租户表；禁止业务代码手写 `WHERE tenant_id`。
  - CI 增加测试：无 `SET LOCAL` 时查询不到数据；不同 tenant 上下文互不可见（§20.3 越权测试）。
  - **PgBouncer**：transaction pooling 模式下先验证 `SET LOCAL` 的生命周期（§15.3 待核对）；不通过则不用 transaction pooling 或改连接策略。
