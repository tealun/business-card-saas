...
| POST `/api/v1/employee/card-exchanges/:requestId/withdraw` | JWT | 发起方撤回仍在待处理的请求；重复撤回幂等 |
| POST `/api/v1/employee/card-exchanges/notifications/subscribe` | JWT | 记录用户已授权的一次性交换通知，事件为 `request_received` 或 `request_accepted` |
...
| GET `/api/v1/wecom/callbacks/login` | 企业微信签名 | 服务商后台「登录授权」页 URL 验证；使用登录授权专用 Token/AESKey |
| POST `/api/v1/wecom/callbacks/login` | 企业微信签名 | 接收并确认登录授权页回调消息；不处理 suite_ticket/create_auth 等第三方应用指令事件 |
