...
- **回调重试**：`callback_events.status` + `retry_count` 驱动;超阈值进死信 + 告警。当前告警通过可选 `WECOM_CALLBACK_ALERT_WEBHOOK_URL` 发送脱敏 webhook payload，不包含密文 callback payload、原始 `event_key` 或 `permanent_code`。
