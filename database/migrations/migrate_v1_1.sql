-- migrate_v1_1.sql
-- 基线迁移：完整建库（schema.sql + rls.sql 在 2026-07-11 的冻结快照）。
-- 已有存量库（表结构已存在）不要执行本文件，用 mark 标记为已应用：
--   node scripts/migrate.cjs mark migrate_v1_1
-- 之后的增量变更一律新建 migrate_vX_Y.sql，禁止回改本文件。
