...
export class CompanyVideoFeatureRepository {
  ...
  async listTenants(search: string, limit: number, offset: number): Promise<{ items: TenantFeatureRecord[]; total: number }> {
    if (!this.hasDatabase()) {
      const values = [...this.tenants.values()].filter((tenant) => tenant.tenantName.includes(search));
    ...
    const result = await this.database!.query<TenantRow & { total_count: string | number | bigint }>(
      `
        ...
        LIMIT $3 OFFSET $4
      `,
      [VIDEO_FEATURE_KEY, search, limit, offset]
    );
    ...
    const result = await this.database!.query<TenantRow>(
      `
        ...
        ON CONFLICT (tenant_id, feature_key) DO UPDATE SET
          ...
          updated_at = now()
        RETURNING tenant_id, $5::text AS tenant_name, enabled, limit_bytes, updated_at
      `,
